...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    for stat in statistics:
        ...
        comp_stat = formula(info, stat)
        players_tuple = tuple((player_id, comp_stat))
        tuple_list.append(players_tuple)

    for num in range(numplayers):
        tuple_list = sorted(tuple_list, key=lambda x: x[1], reverse=True)
        sorted_list.append(tuple_list[num])
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    """
    str_list = []
    master_reader = read_csv_as_nested_dict(info['masterfile'],
                                            info['playerid'], info['separator'], info['quote'])
    for each_id in top_ids_and_stats:
        float_id = str('{:.3f}'.format(each_id[1]))
        first_name = master_reader[each_id[0]][info['firstname']]
        last_name = master_reader[each_id[0]][info['lastname']]
        str_list.append('{} --- {} {}'.format(float_id, first_name, last_name))
    return str_list
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    filter_list = filter_by_year(stats_list, year, info['yearid'])
    top_players = top_player_ids(info, filter_list, formula, numplayers)
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    for each in statistics:
        if each[playerid] not in player_dict:
            stat_dict[playerid] = each[playerid]
            for field in fields:
                stat_dict[field] = int(each[field])
            player_dict[each[playerid]] = dict(stat_dict)
        else:
            for field in fields:
                player_dict[each[playerid]][field] += int(each[field])
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    """
    stats_list = []
    agg_stat = aggregate_by_player_id(read_csv_as_list_dict(info['battingfile'],
                                                            info['separator'], info['quote']),
                                      info['playerid'], info['battingfields'])
    for value in agg_stat.values():
        stats_list.append(value)
    return lookup_player_names(info, top_player_ids(info, stats_list, formula, numplayers))
